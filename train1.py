...
def model_fn(FLAGS, objective, optimizer, metrics):
    ...
    """
    base_model = DenseNet201(weights='K://huawei/src/models/densenet201_weights_tf_dim_ordering_tf_kernels_notop.h5',
                          include_top=False,
                          input_shape=(FLAGS.input_size, FLAGS.input_size, 3),
                          # input_shape=(224, 224, 3),

                             )
    for layer in base_model.layers[:len(base_model.layers)-32]:
    ...
    return model
# m = model_fn(224,  'categorical_crossentropy', adam(), ['acc'])
# m.summary()
...
def train_model(FLAGS):
    ...
    model.fit_generator(
        ...
        datagen.flow(train_sequence, batch_size=FLAGS.batch_size),
